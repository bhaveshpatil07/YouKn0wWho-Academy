...
export default function TopicList() {
  ...
  const completedTopics = getCompletedProblems();
  ...
  return (
    <Box minH="95vh" bg={isLightMode ? "gray.100" : "#212225"}>
      <Head>
        <title>Hetan's Guide | Under Development </title>
        <meta
          name="description"
      ...
      >
        ...
        >
          <Tooltip
            label={`Switch to ${colorMode === "light" ? "dark" : "light"
              } mode.`}
        ...
        >
          <Flex direction={"row"} alignItems="center">
            <Select
              ...
              onChange={(e) => { isLoggedIn && setDifficultyFilter(e.target.value) }}
              size={"sm"}
            ...
            <Select
              ...
              onChange={(e) => { isLoggedIn && setImportanceFilter(e.target.value) }}
              size={"sm"}
              width={{ base: "120px", md: "160px" }}
              borderRadius={"md"}
              boxShadow={"md"}
        ...
        >
          {topicList.map((category, index) => (
            ...
            >
              ...
              {isLoggedIn || index === 0 ? ( // only render TopicAccordion if logged in or first category
                <TopicAccordion
                  category={category}
      ...
      >
        ...
        <ModalContent bg={isLightMode ? "gray.50" : "gray.800"}>
          ...
          <ModalBody p={4}>
            <Input
              ref={initialRef}
              placeholder={isLoggedIn ? "Type to search..." : "Log in to search..."}
              value={searchTerm}
              onChange={(e) => { isLoggedIn ? setSearchTerm(e.target.value) : null }}
            ...
            {filteredTopics.length > 0 ? (
              <>
                ...
                >
                  ...
                  {importanceFilter &&
                    `${difficultyFilter ? "and" : "with"} Importance ${importanceFilter === "1"
                      ? "★☆☆"
                      : importanceFilter === "2"
                        ? "★★☆"
                        : "★★★"
                ...
                {subCategory.topics.filter(matchesFilters).map((topic) => {
                  ...
                  const topicCompleted = async (check) => {
                    ...
                    if (!check) {
                      ...
                      await instance.patch("/progress", { categoryId: category.category_id, subCategoryId: subCategory.sub_category_id, topics: [topic.topic_id] }).then((resp) => {
                        // console.log(resp.data);
                      }).catch((err) => {
                        toast.error(
                          `Error: ` + err.message
                  ...
                  const topicItem = useMemo(() => {
                    return (
                      <Flex key={topic.topic_id}>
                        <TopicItem
                          key={topic.topic_id}
                          topic={topic}
                          isFocused={isFocused}
                          ...
                          handleCheck={topicCompleted}
                        />
                        <Button size={"xs"} isLoading={isLoading} display={isLoading ? "block" : "none"} />
                      </Flex>
                    );
                  }, [topic, isFocused, getTopicDifficultyBgColor, renderImportanceStars, isLightMode, isChecked, topicCompleted]);
...
const TopicItem = React.memo(
  ({
    ...
    const [isChecked, setIsChecked] = useState(isCheckedItem);
      ...
      >
        {isClient &&
          <>
            <Tooltip
              placement="left"
              label={isChecked ? "Mark as not completed" : "Mark as completed"}
              hasArrow
              isOpen={isOpen}
              closeDelay={500}
            >
              <Checkbox
                onChange={(e) => { handleCheck(e.target.checked); setIsChecked(e.target.value); }}
