...
import { completedProblemList, topicList } from "../data/topicList";
...
const TopicData = () => {
  ...
  useEffect(() => {
    if (topic_id) {
      ...
      NProgress.done();
      const allSolvedProblems = getSolvedProblemsByTopic(topic_id);
      setSolvedProblems(allSolvedProblems);
    }
  }, [topic_id]);
  ...
  completedProblemList.forEach((topic) => {
    topic.problems.forEach((problem) => {
      completedProblems[problem] = true;
    });
  });
  ...
  return (
    <Box bg={isLightMode ? "gray.100" : "#212225"}>
      ...
      >
        <Button onClick={() => router.push("/")} leftIcon={<TbPlayerTrackPrevFilled />} shadow={"lg"} size={"sm"} colorScheme='blue' variant='solid'>
          Go to Topic List
        </Button>
        <Head>
                        ...
                        {problems.map((problem, index) => {
                          ...
                          const topicCompleted = async (check) => {
                            ...
                            if (!check) {
                              ...
                              await instance.patch("/progress/problems", { topicId: topic_id, problems: [problem.problem_id] }).then((resp) => {
                                // console.log(resp.data);
                              }).catch((err) => {
                                toast.error(
                                  `Error: ` + err.message
                          ...
                          return (
                            <Tr key={problem.problem_id}>
                              <>
                                ...
                                >
                                  <Checkbox onChange={(e) => topicCompleted(e.target.checked)} defaultChecked={isSolved} title={ isSolved ? "Mark as unsolved" : "Mark as solved"} />
                                </Td>
                                <Td
                                  borderBottom={
                                    isLightMode ? null : "1px solid gray"
